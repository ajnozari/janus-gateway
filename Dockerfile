...
RUN apt-get -y update && apt-get install -y libmicrohttpd-dev \
    ...
    unzip \
    zip \
    lsof wget vim sudo rsync cron default-mysql-client openssh-server supervisor locate gstreamer1.0-tools mplayer valgrind certbot python-certbot-apache
...
RUN apt-get remove -y libnice-dev libnice10 && \
    ...
    apt-get  update && \
    apt-get install -y gtk-doc-tools libgnutls28-dev -t buster-backports  && \
    git clone https://gitlab.freedesktop.org/libnice/libnice.git && \
    cd libnice && \
    git checkout 67807a17ce983a860804d7732aaf7d2fb56150ba && \
    #master commit hash test next
